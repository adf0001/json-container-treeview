...
var addNode = function (el, content, options, childrenContainer) {
	...
	if (typeof name !== "string") throw Error("invalid name, " + name);

	var value = ("value" in content) ? content.value : "value";
	...
	if (
		...
		throw Error("node is not json container");
	}
	...
	if (insert) {
		if (diData instanceof Array) {
			...
			throw Error("name is already existed, " + name);
		}
